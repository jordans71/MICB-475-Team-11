...
PD_metadata = PD_metadata[!is.na(PD_metadata$Disease),]
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = PD_metadata, group = "Disease", daa_method = "DESeq2")
...
pd_control_map <- pathway_heatmap(abundance = abundance_data_filtered_with_description %>% filter(description %in% description_p_0.05$description) %>% column_to_rownames("description"), 
                                         metadata = PD_metadata, 
                                         group = "Disease")
pd_antidepressant_map
ggsave(filename = "pd_control_map.png", pd_control_map, height = 11, width = 17)

# Generate pathway PCA plot
# Please change column_to_rownames() to the feature column if you are not using example dataset
# Please change group to "your_group_column" if you are not using example dataset
pd_control_pca <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = PD_metadata, group = "Disease")
pd_control_pca

ggsave(filename = "pd_control_pca.png", pd_control_pca, height = 8, width = 10)

res =  DEseq2_function(abundance_data_filtered,PD_metadata,"Disease")
...
pd_control_log <- ggplot(data = sig_res, aes(y = reorder(description, sort(as.numeric(log2FoldChange))), x= log2FoldChange, fill = pvalue))+
  geom_bar(stat = "identity")+
  theme_bw()+
  labs(x = "Log2FoldChange", y="Pathways")
pd_control_log

ggsave(filename = "pd_control_log.png", pd_control_log, height = 10, width = 10)
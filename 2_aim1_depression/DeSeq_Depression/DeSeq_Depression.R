...
#### Load data ####
load("~/parkinsons_project_2/DeSeq_Depression/parkinsons_final_depression.RData")
...
res <- results(DESEQ_depression, tidy=TRUE, 
               ...
               contrast = c("depression_binned","High","Low"))
View(res)

## Volcano plot: effect size VS significance
depression_vol <- ggplot(res) +
  geom_point(aes(x=log2FoldChange, y=-log10(padj)))
ggsave('depression_deseq.png',
       depression_vol,
       height = 10, width = 10)
...
ggsave('depression_bar_plot.png',
       depression_bar_plot)

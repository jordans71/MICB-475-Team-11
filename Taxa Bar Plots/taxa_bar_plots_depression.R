...
ggplot(data =data_rel_sum_filtered, aes(depression_binned,mean_rel_abs, fill = Phylum))+
  geom_col()+
  theme_bw()+
...
for (i in levels){
  ...
  df_sum = df %>%
    group_by(depression_binned,legend,Disease,Phylum,Order,Family,Class, Genus) %>%
    summarize(abs = sum(abundance))
...
data_rel_proteobacteria = data_rel_genus %>%
  ...
  group_by(legend,Disease,depression_binned,Phylum, Order, Class, Family,Genus) %>%
...
ggplot(data =data_rel_proteobacteria, aes(depression_binned,mean_rel_abs, fill = Genus))+
  geom_col()+
  theme_bw()+
  theme(axis.text.x = element_text(angle = -90),
        axis.text = element_text(size = 15, face = "bold"),

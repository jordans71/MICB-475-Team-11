...
ggplot(data =data_rel_sum_filtered, aes(Sleep_problems,mean_rel_abs, fill = Phylum))+
  geom_col()+
  theme_bw()+
...
ggplot(data =data_rel_proteobacteria, aes(Sleep_problems,mean_rel_abs, fill = Genus))+
  geom_col()+
  theme_bw()+
  theme(axis.text.x = element_text(angle = -90),
        axis.text = element_text(size = 15, face = "bold"),
